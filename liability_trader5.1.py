...
class LiabilityTrader:
    ...
    v5.0 introduces an execution buffer to filter marginal tenders.
    v5.1 adds a front-loaded first-slice loss rejection safeguard.
    """
    ...
    def __init__(self, api_key, base_url="http://localhost:9999/v1"):
        ...
        self.execution_buffer_per_share = 0.025  # 2.5 cents forced-unwind buffer
        ...
        print(f"[{self._timestamp()}] Liability Trader 5.1 initialized")
    ...
    def evaluate_tender(self, tender):
        ...
        # v5.1 safeguard: reject tenders that are expected to lose money on the
        # very first forced unwind slice (using *only* current top-of-book).
        # This is defensive and compliance-safe (no prediction, no new execution logic).
        abs_qty = abs(int(qty))
        if abs_qty > 0:
            max_order = int(self.max_order_sizes.get(ticker, abs_qty))
            # Match unwind slice sizing: 25% of inventory, rounded like unwind.
            slice_qty = max(1, int(round(0.25 * float(abs_qty))))
            slice_qty = min(slice_qty, abs_qty, max_order)

            if qty > 0:
                # Tender makes us LONG -> first unwind slice is SELL hitting best bid.
                if book.get('bids'):
                    top = book['bids'][0]
                    best_bid = float(top.get('price'))
                    top_liq = int(top.get('quantity', 0) - top.get('quantity_filled', 0))
                    slice_qty = min(slice_qty, max(0, top_liq))
                    if slice_qty > 0:
                        expected_first_slice_pnl = (best_bid - float(price) - 2 * self.transaction_fee) * slice_qty
                        if expected_first_slice_pnl < 0:
                            return (False, 0, "REJECTED: front-loaded unwind loss", [])
            else:
                # Tender makes us SHORT -> first unwind slice is BUY lifting best ask.
                if book.get('asks'):
                    top = book['asks'][0]
                    best_ask = float(top.get('price'))
                    top_liq = int(top.get('quantity', 0) - top.get('quantity_filled', 0))
                    slice_qty = min(slice_qty, max(0, top_liq))
                    if slice_qty > 0:
                        expected_first_slice_pnl = (float(price) - best_ask - 2 * self.transaction_fee) * slice_qty
                        if expected_first_slice_pnl < 0:
                            return (False, 0, "REJECTED: front-loaded unwind loss", [])
    ...
    def _analyze_buy_tender(self, ticker, block_qty, block_price, asks, time_mode, remaining):
        ...
        for level in asks:
            ...
            profit_per_share = (
                ask_price
                - block_price
                - 2 * self.transaction_fee
                - self.execution_buffer_per_share
            )
    ...
    def run(self):
        print(f"[{self._timestamp()}] Starting Liability Trader 5.1...")
...
def main():
    print("=== RCFA Liability Trading Bot 5.1 ===")
