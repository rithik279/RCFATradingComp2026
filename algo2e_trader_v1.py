# -*- coding: utf-8 -*-
# ALGO2e v1 — minimal structural refactor of MODE C v2.3 FINAL
# Goal: run the same maker engine on two symbols (CNR + AC) with per-symbol state.
# Strategy logic is intentionally preserved.
...
# Fee / rebate config (stored only; not used in v1 decision logic)
SYMBOL_CONFIG = {
...
def cap_qty_ctx(session, symbol, side, qty, ctx: ExposureContext):
    ...
    # Reduce-only buys are limited to covering existing short + any open sells.
    # Reduce-only sells are limited to covering existing long + any open buys.
    if side == "BUY":
    ...
    # Reserve reduce-only portion immediately so subsequent orders in the same
    # refresh cycle see updated exposure and cannot collectively oversubscribe.
    ...
    # Find the maximum additional (risk-increasing) qty beyond reduce_only_qty
    # that satisfies global constraints and reserve it.
...
def main():
    ...
    with requests.Session() as s:
        ...
        print(f"[{ts()}] ALGO2e v1 starting: symbols={TRADED_SYMBOLS}, sticky={STICKY_CYCLES}, NET_LIMIT={NET_LIMIT}.")
