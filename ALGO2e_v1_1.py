# -*- coding: utf-8 -*-
# ALGO2e v1.1 — v1 + inventory freeze fix
# Goal: Fix the freeze bug where the bot stops trading when inventory is large.
# Key changes:
# 1) Widened usable inventory band: LIMIT_BUFFER=500 (was 1200), NET_LIMIT=24500 (was 23400)
# 2) Fixed cap_qty to allow reduce-only orders even near limits
# 3) Added Inventory Rescue Mode: forces one-sided unwinding when pos is very large
...
# Fee / rebate config (stored only; not used in v1.1 decision logic)
SYMBOL_CONFIG = {
...
def build_exposure_context(session):
    ...
    # Ensure all traded symbols exist in the breakdown.
...
LIMIT_BUFFER = 500  # Reduced from 1200 to widen usable band
PER_ORDER_MAX = 5000

# Net clamp to prevent NET limit fines (per symbol)
NET_LIMIT = 24500  # Widened from 23400 (POSITION_LIMIT - 1600)
...
# Inventory Rescue Mode: triggers when inventory hits very large levels
RESCUE_POS = 22000       # Trigger threshold
RESCUE_STICKY = 4        # Stay in rescue mode for N cycles
RESCUE_MIN_QTY = 500     # Minimum unwind qty
...
class SymbolState:
    def __init__(self):
        ...
        self.sticky_left = 0
        self.rescue_left = 0  # Inventory rescue mode counter
...
def get_all_positions(session):
    """Fetch positions for all TRADED_SYMBOLS (portfolio snapshot)."""
    r = session.get("http://localhost:9999/v1/securities")
    r.raise_for_status()
    pos = {sym: 0 for sym in TRADED_SYMBOLS}
    for sec in r.json():
        t = sec.get("ticker")
        if t in pos:
...
def get_all_open_orders(session):
    ...
    try:
        ...
        if r.status_code == 401:
            raise SystemExit(
                "401 Unauthorized from /orders. Set a valid RIT API key via env RIT_API_KEY "
...
def cap_qty_ctx(session, symbol, side, qty, ctx: ExposureContext):
    ...
    # 
    # NEW (v1.1): Reduce-only orders bypass global checks but still respect per-symbol limits.
    side = str(side).upper()
    ...
    # --- Per-symbol limits (computed via projected positions) ---
    ...
    # --- NEW (v1.1): Detect reduce-only orders ---
    # If we're already holding inventory in one direction, ANY trade opposite that direction is reduce-only.
    is_reduce_only = False
    if side == "BUY" and pos_sym < 0:
        is_reduce_only = True
    elif side == "SELL" and pos_sym > 0:
        is_reduce_only = True

    # For reduce-only orders, allow full qty up to open-order capacity without global checks.
    if is_reduce_only:
        if side == "BUY":
            reduce_cap = min(int(max_local), abs(int(pos_sym)) + int(open_sell))
        else:
            reduce_cap = min(int(max_local), int(pos_sym) + int(open_buy))
        ...
        if reduce_cap > 0:
            ...
            reserve(reduce_cap)
            return int(reduce_cap)
        else:
            return 0

    # --- Risk-increasing orders: normal global checks apply ---
...
def cap_qty(session, symbol, side, qty):
    # Backwards-compat wrapper (not used by the main loop).
    ctx = build_exposure_context(session)
    return cap_qty_ctx(session, symbol, side, qty, ctx)
...
def place_market(session, symbol, side, qty, ctx):
    # Used only for endgame flattening (preserved behavior).
    qty = cap_qty_ctx(session, symbol, side, qty, ctx)
    if qty <= 0:
        return 0
...
def main():
    ...
    # Validate symbol list matches strict constraints.
    ...
    with requests.Session() as s:
        ...
        print(f"[{ts()}] ALGO2e v1.1 starting: symbols={TRADED_SYMBOLS}, sticky={STICKY_CYCLES}, NET_LIMIT={NET_LIMIT}.")
        ...
        while not SHUTDOWN:
            ...
            for sym in TRADED_SYMBOLS:
                ...
                # --- NEW (v1.1): Inventory Rescue Mode ---
                # Triggers when inventory becomes very large; forces one-sided unwinding.
                if abs(pos) >= RESCUE_POS:
                    st.rescue_left = RESCUE_STICKY
                else:
                    st.rescue_left = max(0, st.rescue_left - 1)

                rescue_mode = st.rescue_left > 0

                if rescue_mode:
                    # Rescue: force immediate cancel, rebuild context, then place one-sided reduce order.
                    cancel_symbol(s, sym)
                    time.sleep(0.10)
                    ctx = build_exposure_context(s)

                    # Refresh pos after cancel.
                    pos = get_position(s, sym)

                    if pos > 0:
                        # Long position: SELL at touch (best ask)
                        unwind_qty = min(
                            PER_ORDER_MAX,
                            max(RESCUE_MIN_QTY, abs(pos) // 4),
                        )
                        place_limit(s, sym, "SELL", unwind_qty, ba, ctx)
                    elif pos < 0:
                        # Short position: BUY at touch (best bid)
                        unwind_qty = min(
                            PER_ORDER_MAX,
                            max(RESCUE_MIN_QTY, abs(pos) // 4),
                        )
                        place_limit(s, sym, "BUY", unwind_qty, bb, ctx)

                    print(
                        f"[{ts()}] {sym} RESCUE pos={pos:+} NET_LIMIT={NET_LIMIT} "
                        f"rescue_left={st.rescue_left} unwind_qty={unwind_qty if pos != 0 else 0}"
                    )
                    st.last_refresh_time = now
                    continue

                # --- Normal Mode C logic (skipped if rescue_mode is active) ---
                ...
                # Portfolio quote budget (minimal scaling) based on global gross headroom.
                ...
                print(
                    ...
                    f"modeC={int(mode_c)} sticky_left={st.sticky_left} cancel={int(need_cancel)} "
                    f"half={half_spread:.4f} sizes=({s1},{s2},{s3}) NET_LIMIT={NET_LIMIT}"
                )

            time.sleep(TICK_SLEEP)

        print(f"[{ts()}] ALGO2e v1.1 stopped.")
