# -*- coding: utf-8 -*-
# ALGO2e v2 — v1 + minimal Harvest Mode (inspired by MODE C v2.3.1)
# Goal: improve post-tick ~150 monetization while preserving:
# - zero fines
# - global net/gross limit safety
# - reservation-based exposure accounting
...
# Fee / rebate config (stored only; not used in v2 decision logic)
SYMBOL_CONFIG = {
...
# Harvest Mode (minimal)
HARVEST_START_TICK = 220
HARVEST_POS_TRIGGER = 6000
HARVEST_STICKY = 8
HARVEST_HALF_SPREAD_ADD = 0.003
HARVEST_UNWIND_BIAS = 0.01
...
def compute_quotes(bb, ba, pos, half_spread, unwind_bias=0.0):
    mid = (bb + ba) / 2.0
    inv = max(-1.0, min(1.0, pos / 20000.0))
    skew = INVENTORY_SKEW_MAX * inv

    bid_u = mid - half_spread - max(0.0, skew)
    ask_u = mid + half_spread - min(0.0, skew)

    if pos < 0:
        bid_u += float(unwind_bias)
    elif pos > 0:
        ask_u -= float(unwind_bias)
...
def main():
    ...
    with requests.Session() as s:
        ...
        print(
            f"[{ts()}] ALGO2e v2 starting: symbols={TRADED_SYMBOLS}, sticky={STICKY_CYCLES}, NET_LIMIT={NET_LIMIT}."
        )
        ...
        while not SHUTDOWN:
            ...
            for sym in TRADED_SYMBOLS:
                ...
                # Harvest triggers (simple + safe)
                harvest_triggered = ((tick >= HARVEST_START_TICK) and (abs(pos) > HARVEST_POS_TRIGGER)) or (
                ...
                if harvest_mode:
                    ...
                    unwind_bias = HARVEST_UNWIND_BIAS

                bid, ask, _ = compute_quotes(bb, ba, pos, half_spread, unwind_bias=unwind_bias)
                ...
                print(
                    ...
                    f"modeC={int(mode_c)} sticky_left={st.sticky_left} harvest={int(harvest_mode)} "
                    f"harvest_left={st.harvest_left} cancel={int(need_cancel)} "
                    f"half={half_spread:.4f} sizes=({s1},{s2},{s3})"
                )

            time.sleep(TICK_SLEEP)

        print(f"[{ts()}] ALGO2e v2 stopped.")
